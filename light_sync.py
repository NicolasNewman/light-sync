...
def main(argv):
    ...
    parser.add_argument("-D", "--dimlightsinsteadofturnoff",
                        help="Dim lights or Turn OFF on dark screens (default true - DIM)")
    parser.add_argument("-t", "--transitiontime", help="Transition time, default 1")
    parser.add_argument("-y", "--transitiontype",
                        help="Transition type, default 1 (0 = instant, 1 = comfortable, 2 = smooth)")
    ...
    parser.add_argument("-s", "--framematchsensitivity",
                        help="Use to skip similar frames in a row [0-1] default: 0.008")
    parser.add_argument("-S", "--colorskipsensitivity", help="Skip frame if color is similar [0-256], default 10")
    parser.add_argument("-g", "--brightnessskipsensitivity",
                        help="Skip frame if brightness diff is less [0-256], default 10")
    ...
    parser.add_argument("-z", "--shrinkframesize",
                        help="Frame capture shrinked size in pixel for better performance (default 100)")

    # Auto adjust performance
    parser.add_argument("-A", "--autoadjustperformance", help="Auto adjust script performance (default True)")
    parser.add_argument("-Af", "--autoadjustperformancelowfpscount",
                        help="Number of low fps in a row to trigger performance adjust (default 2)")
    ...
    parser.add_argument("-fci", "--colorflickerinput",
                        help="Color flicker detection required input frames count (default 4)")
    parser.add_argument("-foi", "--onoffflickerinput",
                        help="On/Off flicker detection required input frames count (default 8)")
    parser.add_argument("-fcc", "--colorflickercount",
                        help="Number of frames that match color flicker condition required to trigger correction (default 2)")
    parser.add_argument("-foc", "--onoffflickercount",
                        help="Number of frames that match on/off flicker condition required to trigger correction (default 2)")
    parser.add_argument("-fonz", "--onoffflickerminnzcorrection",
    ...
    with mss.mss() as sct:
        ...
        while 'Screen capturing':
            ...
            current_brightness = FRAME_COLOR_LIB.calculate_frame_brightness(masked_frame,
                                                                            dim_brightness, starting_brightness,
                                                                            min_non_zero_count * min_non_zero_count_flicker_correction,
                                                                            max_non_zero_count)
                ...
                if (result_color.diff_from_prev is not None and \
                    ...
                    if current_buffer_size >= color_flicker_prevent_required_input_frames_count:
                        ...
                        if color_flicker_count >= color_flicker_count_threshold:
                            ...
                            # print('COLOR FLICKER AVOIDED | new K: {0}'.format(
                            #     number_of_k_means_clusters - k_means_flicker_correction))
                    ...
                    if not dim_lights_instead_of_turn_off and go_dark:
                        ...
                        if current_buffer_size >= on_off_flicker_prevent_required_input_frames_count:
                            ...
                            if on_off_count >= on_off_flicker_count_threshold:
                                ...
                                # print('ON/OFF FLICKER AVOIDED | new min nz: {0}'.format(
                                #     min_non_zero_count * min_non_zero_count_flicker_correction))
